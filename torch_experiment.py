...
if __name__ == "__main__":
    ...
    # optimiser_av = optim.Adam(net.G.parameters(), lr=0.0002)
    optimiser_g = optim.Adam(net.G.parameters(), lr=0.0002)
    optimiser_d = optim.Adam(net.D.parameters(), lr=0.0002)

    # start training
    epoch_report = {}
    until_epoch = args.start_from_checkpoint + 1 + n_epochs
    for current_epoch in range(args.start_from_checkpoint + 1, until_epoch):
        bar = tqdm.trange(updates_per_epoch)
        epoch_report['epoch'] = '[%d/%d]' % (current_epoch, until_epoch)
        ...
        for update in bar:
            ...
            states_nonep = states_ep.view(EP_LEN * BATCH_SIZE, -1)
            ...
            if train_d_switch is True:
                label.data.fill_(real_label)

                obs_out_nonep = obs_out.view(EP_LEN * BATCH_SIZE,
            ...
            if train_g_switch is True:
                ...
                label.data.fill_(real_label)
                out_d_g = net.D(obs_sample)
                err_g = criterion_gan(out_d_g, label)
                losses.append(err_g)
            ...
            if train_av_switch is True:
                ...
                obs_exp_nonep = obs_expectation.view(EP_LEN * BATCH_SIZE,
                                                     obs_out.size(2), obs_out.size(3), obs_out.size(4))
                ...
                if update % 10 == 0:
                    ...
                    torch_utils.batch_to_sequence(joint, fpath='{}/images/training_sum_{}.gif'.format(output_dir, current_epoch))
            ...
            if update % 100 == 0:
                ...
                if update % 500 == 0:
                    ...
                    torch_utils.batch_to_sequence(joint, fpath='%{}/images/training_recon_{}.gif'.format(output_dir, current_epoch))
